...
export const ClientApi = {
  getTemperature: async function (cancel = false) {
    const response = await api.request({
      ...
      signal: cancel
        ? cancelApiObject[this.getTemperature.name].handleRequestCancellation()
            .signal
        : undefined,
    });
  ...
  getHeat: async function (cancel = false) {
    const response = await api.request({
      ...
      method: "GET",
      signal: cancel
        ? cancelApiObject[this.getHeat.name].handleRequestCancellation().signal
        : undefined,
    });
    return roundNumber(response.data);
  },
  ...
  getEnvironment: async function (cancel = false) {
    const response = await api.request({
      ...
      method: "GET",
      signal: cancel
        ? cancelApiObject[this.getEnvironment.name].handleRequestCancellation()
            .signal
        : undefined,
    });
