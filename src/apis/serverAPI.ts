...
export const API = {
	...
	getSwitches: async function (cancel = false) {
		try {
			const response = await serverApi().request({
				...
				method: "GET",
				signal: cancel
					? cancelApiObject[this.getSwitches.name].handleRequestCancellation()
							.signal
					: undefined,
			});
			return response.data;
	...
	putSwitch: async function (concreteSwitch: switch_, cancel = false) {
		try {
			const response = await serverApi().request({
				url: `/switch`,
				method: "PUT",
				...
				data: concreteSwitch,
				signal: cancel
					? cancelApiObject[this.putSwitch.name].handleRequestCancellation()
							.signal
					: undefined,
			});
			return response.data;
	...
	putTime: async function (dateTime: Date, cancel = false) {
		const response = await serverApi().request({
			url: "/setTime",
			method: "PUT",
			data: {
			...
			signal: cancel
				? cancelApiObject[this.putTime.name].handleRequestCancellation().signal
				: undefined,
		});
		return response.data;
	},
