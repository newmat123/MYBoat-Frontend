...
export async function scheduleAnchorReminder(id: number) {
	const hasAccess = await initNotifications();
	if (!hasAccess) {
		return;
	}
		...
		notifications: [
			{
				id,
				title: "Anchor Light",
				body: "Your anchor light is enabled. Remember to turn it off when its not needed.",
				schedule: {
					on: {
						hour: 19,
						minute: 0,
...
export async function cancelAnchorReminder(id: number) {
	const hasAccess = await initNotifications();
	if (!hasAccess) {
		return;
	}
